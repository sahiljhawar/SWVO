...
if __name__ == '__main__':
    ...
    log_dir = Path(os.environ.get('DAILY_DOWNLOAD_LOG_DIR')) / f'{datetime.today().year}' / f'{datetime.today().month}'

    log_dir.mkdir(parents=True, exist_ok=True)

    logging.basicConfig(filename=str(log_dir / f'daily_downloads_log_{datetime.today().strftime("%Y%m%d_T%H0000")}.log'),
                    filemode='w',
    ...
    logging.info(f'Target time from {date_yesterday_start} to {datetime.today()}')
