...
class RBMDS:
    ...
    def update_from_dict(self, source_dict: dict[VariableLiteral, NDArray[np.floating] | list[dt.datetime]]) -> RBMDS:
        ...
        RuntimeError
            If the RBMDS is in file loading mode and dictionary loading is not enabled.

        """
        if self._file_loading_mode and not self._enable_dict_loading:
            msg = "RBMDS is in file loading mode. Cannot update from dictionary. To use dictionary-based loading, set `enable_dict_loading=True` during initialization."
    ...
    def set_file_path_stem(self, file_path_stem: Path) -> RBMDS:
        self._file_path_stem = file_path_stem
        return self

    def set_file_name_stem(self, file_name_stem: Path) -> RBMDS:
        self._file_path_stem = file_name_stem
        return self

    def set_file_cadence(self, file_cadence: FileCadenceEnum) -> RBMDS:
    ...
    def _load_variable_netcdf(self, var: Variable | VariableEnum) -> None:
        ...
        for var_name in var_names_stored:
            ...
            rbm_var_names = self._get_rbm_name(var_name, self._mfm.mfm_name)  # ty:ignore[invalid-argument-type]
    ...
    def __eq__(self, other: RBMDS) -> bool:  # type :ignore[override]
        if (
    ...
    def get_different_variables(self, rbm_other: RBMDS) -> list[str]:
