...
class RingCurrentCheck(BaseFileCheck):
    ...
    def _extract_date_from_file(file):
        ...
        timestamp = dt.datetime.strptime(timestamp, "%Y%m%d%TH%M")
        return timestamp
    ...
    def run_check(self, date=None):
        ...
        if not success:
            logging.info("Ring current outputs for date {} not found...sending notification email".format(date))
        ...
        if success:
            logging.info("Ring Current outputs for date {} found!!".format(date))
