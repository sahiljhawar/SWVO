...
class TestReadEnsembleKp(object):
    ...
    def test_init_folder_found(self, mocker):
        ...
        try:
            reader = KPEnsembleReader(wp3_output_folder=folder, ensemble_sub_folder=sub_folder)
    ...
    def test_init_folder_not_found(self, mocker):
        ...
        sub_folder = "/not/existing/sub/folder/"
        try:
            KPEnsembleReader(wp3_output_folder=folder, ensemble_sub_folder=sub_folder)
            assert False
        except FileNotFoundError:
            assert True
