...
def test_initialization_with_env_var(swomni):
...
def test_download_and_process(swomni, mocker):
    ...
    swomni.download_and_process(start_time, end_time, verbose=True)
...
def test_process_single_file(swomni):
...
def test_read_single_file(swomni):
...
def test_invalid_cadence(swomni):
