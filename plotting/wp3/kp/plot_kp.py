...
def add_subplot(ax, data, title=None, rotation=0, title_font=9, xlabel_fontsize=14,
                ylabel_fontsize=20, ylim=(-0.1, 9.1), ylabel=r"$K_{p}$", cadance=3):
    # PLOT
    barcolors = add_bar_color(data, list(data.keys())[0])
    data.plot(kind="bar", ax=ax, edgecolor=['k'] * len(data), color=[barcolors],
    ...
    # Y-AXIS
    ax.set_ylim(ylim)
    ylabels = [i for i in range(10) if i % 2 == 0]
    ...
    def map_dates(x):
        if x.hour % cadance != 0:
            return ""
        if (x.hour % cadance == 0) and (x.minute == 0):
    ...
    xlabels = list(data.index.map(lambda x: map_dates(x)))
    ax.set_xticklabels(labels=xlabels, rotation=rotation, fontsize=xlabel_fontsize)
...
def plot_forecast(model_data):
    ...
    add_subplot(ax, data=model_data[["kp"]], title=None,
                cadance=(model_data.index[1] - model_data.index[0]).seconds // 3600)
...
if __name__ == "__main__":
    ...
    data_niemegk, date = reader.read("niemegk")
    ...
    data_swpc, date = reader.read("swpc")
    ...
    data_swift, date = reader.read("swift")
